(ns kasane.cos
  "PDF COS object parser — the hybrid path from ADR-2606272100.

   PDF is not a linear struct grammar, so this is a hand-written recursive
   descent reader over the byte stream (dict/array/string/name/number/ref/
   stream). It is still pure (no external library): stream filters route
   to kasane.codec (FlateDecode → our pure inflate). Token/operator handling
   is data-driven where it can be.
   ...
   streams and DCTDecode/JPX image pixels are not decoded (opaque)."
  (:require [kasane.codec :as codec]
...
(defn decode-stream
  ...
  (let [f  (resolve-ref objs (:Filter (:dict stream)))
    ...
    (reduce (fn [b filt] (case filt :FlateDecode (codec/decode :flate b) b))
            (:raw stream) fs)))

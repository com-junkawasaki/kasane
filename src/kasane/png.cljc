(ns kasane.png
  "PNG decode: chunk grammar (kasane.decode) + IDAT zlib (pure inflate) +
   scanline unfiltering. Demonstrates the 'free' format from ADR-2606272100 —
   the chunk structure is data (png.edn) and IDAT reuses the inflate that
   already backs PSD/PDF. R0 supports bit-depth 8/16; sub-byte depths (1/2/4)
...
(defn parse
  "Parse PNG `data` with `grammar` (png.edn). Returns
   {:ihdr {...} :pixels [...] :chunks [...]}. Pixels are *not* meant to be
   inlined into :kasane/doc — see kasane.normalize."
  [grammar data]
  (let [raw    (d/decode grammar data)
    ...
    (let [chunks (:chunks raw)
          ...
          inflated (codec/decode :flate idat)]
      {:ihdr   ihdr
       :chunks chunks
